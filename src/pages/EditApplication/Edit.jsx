...
export default function EditApplicationForm() {
    ...
    const { passportNo } = useParams();  // Get passportNo from the URL params if you're using React Router
    ...
    const [formData, setFormData] =  useState({
      ...
      const formatDateForMySQL = (isoDate) => {
        return isoDate ? isoDate.split("T")[0] : ""; // Ensure it's not null or undefined
      };
      
      useEffect(() => {
        // Fetch the application data for editing using the passportNo
        const fetchApplicationData = async () => {
          try {
            const token = localStorage.getItem("token");
            const response = await axios.get(`/application/getapplicant/${passportNo}`, {
              headers: {
            ...
            if (response.status === 200) {
              ...
              // Format date fields properly before setting state
              setFormData({
                ...fetchedData,
                dateOfBirth: formatDateForMySQL(fetchedData.dateOfBirth),
                dateOfIssue: formatDateForMySQL(fetchedData.dateOfIssue),
                dateOfExpiry: formatDateForMySQL(fetchedData.dateOfExpiry),
                signedOn: formatDateForMySQL(fetchedData.signedOn),
                certifiedDate: formatDateForMySQL(fetchedData.certifiedDate),
                relativeBirthDate: formatDateForMySQL(fetchedData.relativeBirthDate),
                date: formatDateForMySQL(fetchedData.date),
              });
      
              // Set photo previews if they exist in the response
              setPreviews({
                regular: fetchedData.regularPhoto || null,
                fullSize: fetchedData.fullSizePhoto || null,
                passport: fetchedData.passportPhoto || null,
              });
      ...
      showConfirmationToast(async () => {
        ...
        try {
          ...
          const response = await axios.put(
            `/application/updateapplication/${passportNo}`,
            formDataObj,
            {
              headers: {
          ...
          if (response.status === 200) {
            toast.success("Application updated successfully!", { position: "top-right" });
          ...
          toast.error("Error updating application.", { position: "top-right" });
        } finally {
          setLoading(false);
        }
      });
                ...
                <Card className="mb-4">
                  <Card.Body>
                    <Row className="g-3">
                      ...
                      <Col md={12}>
                        <Form.Group className="d-flex gap-3 check-bx-cont align-items-center">
                          <Form.Check
                            type="checkbox"
                            id="ironing"
                            label="Ironing"
